...
public class GUIProducto {
	...
	public void crearArrayRequeridos(){
		...
		//arrayTextRequeridos.add(tCodigoMarca);
	...
	/*public void guardar(){
		...
		if (validarRegistro(arrayTextRequeridos, sShell)){
			...
			try {
				...
				llamarProcedimientoAlmacenado.setString(6, tCodigoMarca.getText());
				llamarProcedimientoAlmacenado.setString(7, tCodigoCategoria.getText());
				...
				if (lFoto.getImage() != null) {
					...
					llamarProcedimientoAlmacenado.setBinaryStream( 9, canalEntrada, (int)imagen.length() );
	...
	/*public void eliminar(){
		...
		try {
			...
			mensajeConfirmacion(sShell, "Esta seguro que desea eliminar este procducto?");
	...
	/*public void consultar(){
		...
		try {
			...
			String sentenciaSQL = "Select P.Descripcion,P.IvaIncluido,P.StockMinimo,P.Stock,P.IdMarca,M.Marca,P.IdCategoria,C.Categoria,P.Observaciones,P.Foto "
				+ "From Productos P, Marcas M, Categorias C "
				+ "Where idProducto = ? and M.IdMarca = P.IdMarca and C.IdCategoria = P.IdCategoria";
			...
			if (resultado.next()) { //Se extraen los elementos
				...
				tCodigoMarca.setText(devuelveCadenaVaciaParaNulo(resultado.getString(5)));
				tMarca.setText(devuelveCadenaVaciaParaNulo(resultado.getString(6)));
				tCodigoCategoria.setText(devuelveCadenaVaciaParaNulo(resultado.getString(7)));
				tCategoria.setText(devuelveCadenaVaciaParaNulo(resultado.getString(8)));
				taObservaciones.setText(devuelveCadenaVaciaParaNulo(resultado.getString(9)));
				Blob imagenBaseDatos  = resultado.getBlob(10);
	...
	/*public void agregarEventos(){
		...
		tCodigoMarca.addVerifyListener(getValidarEntradaNumeros());
		tCodigoMarca.addFocusListener(this);
		...
		tMarca.addVerifyListener(getConvertirMayusculaLetras());
		...
		tCodigoMarca.addTraverseListener(new TraverseListener() {
			public void keyTraversed(TraverseEvent e) {
				if (e.detail == SWT.TRAVERSE_TAB_NEXT || e.detail == SWT.TRAVERSE_TAB_PREVIOUS ) {

					if (!tCodigoMarca.getText().isEmpty()) {

						String marca = buscarInformacion("Marcas", "idMarca = " + tCodigoMarca.getText());

						if (marca != null) {

							tMarca.setText(marca);
							taObservaciones.forceFocus();

						} else {

							mensajeError(sShell, "Código de marca no registrado");
							tCodigoMarca.setText("");
							e.doit = false;
	...
	/*public void agregarConfiguraciones(){
		...
		tCodigoMarca.setTextLimit(4);
		tMarca.setTextLimit(60);
		tCategoria.setTextLimit(60);
		
		
		sShell.setImage(new Image(Display.getCurrent(), "D:/Backup/workspace/SIPY/imagenes/InventarioProductos.gif"));
